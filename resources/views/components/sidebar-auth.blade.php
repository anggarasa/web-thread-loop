<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
    <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50">
       ...
       <ul class="space-y-2 font-medium">
          <li>
             <a href="/Home" class="flex items-center p-4 {{ request()->is('Home') ? 'bg-black text-white' : 'text-gray-900' }} text-gray-900 rounded-lg hover:bg-black hover:text-white group">
          ...
          <li>
             <a href="/cari" class="flex items-center p-4 {{ request()->is('cari') ? 'bg-black text-white' : 'text-gray-900' }} rounded-lg hover:bg-black hover:text-white group">
                <ion-icon class="w-7 h-7 font-bold" name="search-outline"></ion-icon>
                <span class="flex-1 ms-3 whitespace-nowrap">Cari</span>
             </a>
          </li>
          <li>
             <a data-modal-toggle="modalPosting" data-modal-target="modalPosting" class="flex items-center p-4 text-gray-900 rounded-lg hover:bg-black hover:text-white group">
                <ion-icon class="w-7 h-7 font-bold" name="cloud-upload-outline"></ion-icon>
                <span class="flex-1 ms-3 whitespace-nowrap">Posting</span>
             </a>
          </li>
          <li>
            <a href="/activity" class="relative flex items-center p-4 {{ request()->is('activity') ? 'bg-black text-white' : 'text-gray-900' }} text-gray-900 rounded-lg hover:bg-black hover:text-white group">
          ...
          <li>
            <a href="/profile-user" class="flex items-center p-4 {{ request()->is('profile-user') ? 'bg-black text-white' : 'text-gray-900' }} rounded-lg hover:bg-black hover:text-white group">
...
<div class="fixed md:hidden lg:hidden z-40 inset-x-0 bottom-0 bg-white shadow-md">
   <div class="flex justify-around items-center py-2">
     <a href="/Home" class="flex flex-col items-center {{ request()->is('Home') ? 'text-black' : 'text-gray-600' }} hover:text-black">
      <ion-icon class="w-7 h-7 font-bold" name="home-outline"></ion-icon>
       <span class="text-xs">Home</span>
     </a>
     <a href="/cari" class="flex flex-col items-center {{ request()->is('cari') ? 'text-black' : 'text-gray-600' }} hover:text-black">
      <ion-icon class="w-7 h-7 font-bold" name="search-outline"></ion-icon>
     ...
     <a href="/activity" class="flex flex-col items-center relative {{ request()->is('activity') ? 'text-black' : 'text-gray-600' }} hover:text-black">
      <div class="relative">
        <ion-icon class="w-7 h-7 font-bold" name="heart-outline"></ion-icon>
        @if(isset($hasNewActivities) && $hasNewActivities)
     ...
     <a href="/profile-user" class="flex flex-col items-center {{ request()->is('profile-user') ? 'text-black' : 'text-gray-600' }} hover:text-black">
